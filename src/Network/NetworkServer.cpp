...
namespace Eternal
{
	...
	{
		NetworkServerClientConnection::NetworkServerClientConnection(_In_ uint32_t InSendBufferSize, _In_ uint32_t InReceiveBufferSize)
		{
			_SendBuffer.reserve(InSendBufferSize);
			_ReceiveBuffer.reserve(InReceiveBufferSize);
		}

		void NetworkServerClientConnection::Send(_In_ const uint8_t* InBlock, _In_ uint64_t InSize)
		{
			if constexpr (ServerStrategy == NetworkStrategy::NETWORKSTRATEGY_IMMEDIATE)
			{
				...
				uint32_t LastBlockEnd = Math::Min(CurrentBlockBegin + _SendBuffer.capacity(), InSize - 1);
		...
		uint32_t NetworkServerClientConnection::Receive(_Out_ uint8_t* OutBlock, _In_ uint64_t InSize)
		{
			if (InSize <= 0)
				return 0u;
